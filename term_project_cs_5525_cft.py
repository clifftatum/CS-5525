...
if __name__ == '__main__':
    ...
    keep_ind, perc_detected_removed = eda.detect_and_remove_outliers(
    ...
    reg_target = 'BandWidth'
    drop_these_rfa.remove(reg_target)
    df[reg_target] = df_init[reg_target]


    x_train_reg, x_test_reg, y_train_reg, y_test_reg = \
        eda.split_80_20(df=df.copy(deep=True).drop(columns=['target_buy_sell_performance']), target='BandWidth')
    ...
    linear_reg_results_rfa,fig_rfa\
    ...
    df_final_res, figs, df_predictions = ra.compare_regression_models(model_a=linear_reg_model_blr,
                                                                      ...
                                                                      show=True, target_name='BandWidth')
    for f in figs:
        fig_pack.append(f)
        f.show()
